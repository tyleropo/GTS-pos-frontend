...
export function generatePurchaseOrderPDF(purchaseOrder: APIPurchaseOrder) {
    const doc = new jsPDF();
    ...
    doc.text(`PO Number: ${purchaseOrder.po_number}`, 20, 35);
    doc.text(
        `Date: ${purchaseOrder.created_at ? new Date(purchaseOrder.created_at).toLocaleDateString() : "N/A"}`,
        20,
        42
    );
    doc.text(
    ...
    const supplier = purchaseOrder.supplier as any;
    ...
    if (purchaseOrder.expected_at) {
        ...
        doc.setFont("helvetica", "normal");
        doc.text(new Date(purchaseOrder.expected_at).toLocaleDateString(), 120, 67);
    }
    ...
    doc.text(`$${purchaseOrder.subtotal.toFixed(2)}`, 180, totalsStartY, { align: "right" });

    doc.text("Tax:", 140, totalsStartY + 7);
    doc.text(`$${purchaseOrder.tax.toFixed(2)}`, 180, totalsStartY + 7, { align: "right" });

    doc.setFont("helvetica", "bold");
    doc.setFontSize(11);
    doc.text("Total:", 140, totalsStartY + 15);
    doc.text(`$${purchaseOrder.total.toFixed(2)}`, 180, totalsStartY + 15, { align: "right" });
